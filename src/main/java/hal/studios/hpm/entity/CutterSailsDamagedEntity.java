...
public class CutterSailsDamagedEntity extends PathfinderMob {
	public CutterSailsDamagedEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(HpmModEntities.CUTTER_SAILS_DAMAGED.get(), world);
	}

	public CutterSailsDamagedEntity(EntityType<CutterSailsDamagedEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		DamagedcutteroninitialspawnProcedure.execute(this);
		return retval;
	}
			...
			if (sourceentity instanceof ServerPlayer serverPlayer) {
				NetworkHooks.openGui(serverPlayer, new MenuProvider() {
					...
					public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
						...
						packetBuffer.writeVarInt(CutterSailsDamagedEntity.this.getId());
